-- as before, but using a type family instead of a fundep
...
--   we'll use MPTCs and a type family to lock everything down correctly

class EvalHask a where
  type HaskType a :: *
  evalHask :: Term a -> HaskType a

instance EvalHask TBool where
  type HaskType TBool = Bool
  evalHask T = True
  evalHask F = False
  evalHask x = evalHask (eval x)

instance EvalHask TNat where
  type HaskType TNat = Int
  evalHask Z = 0
